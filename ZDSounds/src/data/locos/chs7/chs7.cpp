...
void CHS7::readRAMValues(const RAM& ram, const ROM& rom) {
	...
	ReadProcessMemory(pHandle, (LPCVOID)rom.GetAddress(rom.addressesSpecific["locoVoltage"]), &this->voltageLoco, 4, NULL);
	ReadProcessMemory(pHandle, (LPCVOID)rom.GetAddress(rom.addressesSpecific["blinds"]), &this->areBlindsOpened, 1, NULL);
	ReadProcessMemory(pHandle, (LPCVOID)rom.GetAddress(rom.addressesSpecific["reversor"]), &this->reversorState, 1, NULL);
	ReadProcessMemory(pHandle, (LPCVOID)rom.GetAddress(rom.addressesSpecific["position1"]), &this->positionSection1, 1, NULL);
	ReadProcessMemory(pHandle, (LPCVOID)rom.GetAddress(rom.addressesSpecific["epbSensorPressure"]), &this->epbSensorPressure, 4, NULL);
	ReadProcessMemory(pHandle, (LPCVOID)rom.GetAddress(rom.addressesSpecific["mainSwitch"]), &this->isMainSwitchActive.current, 1, NULL);

	// ÌÊ 1
	float mk0 = 0;
	ReadProcessMemory(pHandle, (LPCVOID)rom.GetAddress(rom.addressesSpecific["mks"]["mk0"]), &mk0, 4, NULL);
	this->areMKsActive[0] = mk0 > 0;

	// ÌÊ 2
	uint8_t mk1Mode = 0;	// ñîñòîÿíèÿ ÌÊ 1-2 
	ReadProcessMemory(pHandle, (LPCVOID)rom.GetAddress(rom.addressesSpecific["mks"]["mk1Mode"]), &mk1Mode, 1, NULL);
