...
var
  ...
  AutoInformDoorFlag: Boolean;
  // Флаг который переключает на закрытеи дверей при V=0
  SAVPBaseObjectsCount: Integer;
  BaseInfoTrack: Array [0 .. 500] of Integer;
  SAVPBaseInfoName1: Array [0 .. 500] of String;
  // Первый столбец файла ЭК [САУТ, УСАВПП, САВПЭ]
  SAVPBaseInfoName2: Array [0 .. 500] of String;
  // Второй столбец файла ЭК [САУТ, УСАВПП, САВПЭ]
  SAVPBaseInfoName3: Array [0 .. 500] of String;
  // Третий столбец файла ЭК [САВПЭ]
  ...
  SAVP_EK_NewSystem: Boolean;
  // Переменная для определения, новая-ли система ЭК маршрута (*.TWS, *.txt)
...
begin
  ...
  begin
    ...
    SAVPBaseInfoName1[I] := '';
    SAVPBaseInfoName2[I] := '';
    if Trim(SAVPBaseInfoName1[I + 1]) = '' then
      break;
  end;
...
begin
  ...
  begin
    ...
    if Trim(scSAVPBaseInfoName[I + 1]) = '' then
      break;
  end;
  scBaseInfoCount := 0;
end;
...
begin
  ...
  begin
    ...
    NatureOrdinats2[I] := 0;
    NatureTracks2[I] := 0;
    if Trim(NatureBaseName[I + 1]) = '' then
      break;
  end;
...
// ------------------------------------------------------------------------------//
// function ConvertOldUSAVPNameToNewResName(Str: String) : String; // no uses
// begin
// Str := LowerCase(Str);
// Result := '0';
...
begin
  ...
  begin
    ...
    begin
      if Pos('Zvon', ObjectsList[1]) > 0 then
      begin
...
begin
  ...
  begin
    ...
    if ObjectsList.Count >= 2 then
    begin
      if Pos('nature', ObjectsList[0]) > 0 then
      begin
...
// ------------------------------------------------------------------------------//
// procedure GetSAVPObjectsBaseFromFile(filename: String; newSystem: Boolean);  // no uses
// var
// FS: TFileStream;
// FileText: String;
// FileLinesList: TStringList;
...
begin
  ...
  begin
    ...
    else if ZvonokVolume > ZvonokVolumeDest then
      ZvonokVolume := ZvonokVolume - 0.001 * MainCycleFreq;

    if ZvonokVolume > 1 then
      ZvonokVolume := 1
...
begin
  ...
  begin
    const
      ...
      // GetSAVPObjectsBaseFromFile(infoFile, False);
    GetSAVPPereezdOrdinats(infoFile);
    GetSAVPNatureObjectsOrdinats(infoFile);
  end;
