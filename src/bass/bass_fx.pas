...
uses BASS, Math;
// Math is used for BASS_BFX_Linear2dB/dB2Linear functions below
...
{$ENDIF}
  // BASS_FX Version
function BASS_FX_GetVersion(): DWORD; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
...
const
  ...
  BASS_FX_BFX_REVERB = $10005;
  // Reverb							/ 2 channels max	(deprecated)
  BASS_FX_BFX_LPF = $10006;
...
type
  ...
  TBASS_BFX_ENV_NODES = array [0 .. maxInt div sizeOf(BASS_BFX_ENV_NODE) - 1]
    of BASS_BFX_ENV_NODE;
...
const
  ...
  BASS_ATTRIB_TEMPO_OPTION_USE_AA_FILTER = $10010;
  // TRUE (default) / FALSE (default for multi-channel on mobile devices for lower CPU usage)
  BASS_ATTRIB_TEMPO_OPTION_AA_FILTER_LENGTH = $10011;
  // 32 default (8 .. 128 taps)
  BASS_ATTRIB_TEMPO_OPTION_USE_QUICKALGO = $10012;
...
function BASS_FX_TempoCreate(chan, flags: DWORD): HSTREAM; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
function BASS_FX_TempoGetSource(chan: HSTREAM): DWORD; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
function BASS_FX_TempoGetRateRatio(chan: HSTREAM): Single; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
...
function BASS_FX_ReverseCreate(chan: DWORD; dec_block: Single; flags: DWORD)
  : HSTREAM; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
function BASS_FX_ReverseGetSource(chan: HSTREAM): DWORD; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
...
const
  ...
  // if in use, then you can do other processing while detection's in progress. Available only in Windows platforms (BPM/Beat)
  BASS_FX_BPM_MULT2 = 2;
...
type
  BPMPROC = procedure(chan: DWORD; bpm: Single; user: Pointer);
  {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
  BPMPROGRESSPROC = procedure(chan: DWORD; percent: Single; user: Pointer);
  {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
  BPMPROCESSPROC = BPMPROGRESSPROC; // back-compatibility

function BASS_FX_BPM_DecodeGet(chan: DWORD; startSec, endSec: Double;
  minMaxBPM, flags: DWORD; proc: BPMPROGRESSPROC; user: Pointer): Single;
{$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
function BASS_FX_BPM_CallbackSet(handle: DWORD; proc: BPMPROC; period: Double;
  minMaxBPM, flags: DWORD; user: Pointer): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
function BASS_FX_BPM_CallbackReset(handle: DWORD): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
function BASS_FX_BPM_Translate(handle: DWORD; val2tran: Single; trans: DWORD)
  : Single; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll; // deprecated
function BASS_FX_BPM_Free(handle: DWORD): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
...
type
  BPMBEATPROC = procedure(chan: DWORD; beatpos: Double; user: Pointer);
  {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};

function BASS_FX_BPM_BeatCallbackSet(handle: DWORD; proc: BPMBEATPROC;
  user: Pointer): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
function BASS_FX_BPM_BeatCallbackReset(handle: DWORD): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
function BASS_FX_BPM_BeatDecodeGet(chan: DWORD; startSec, endSec: Double;
  flags: DWORD; proc: BPMBEATPROC; user: Pointer): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
function BASS_FX_BPM_BeatSetParameters(handle: DWORD;
  bandwidth, centerfreq, beat_rtime: Single): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
function BASS_FX_BPM_BeatGetParameters(handle: DWORD;
  var bandwidth, centerfreq, beat_rtime: Single): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
function BASS_FX_BPM_BeatFree(handle: DWORD): BOOL; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
external bass_fxdll;
