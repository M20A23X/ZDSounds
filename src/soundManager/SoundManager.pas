...
procedure DecodeResAndPlay(FileName: String; var FlagName: Boolean;
  var PCharName: PChar; var ChannelName: Cardinal; var ResPotok: TMemoryStream;
...
var
  ...
  // Каналы перестука тележек локомотива (Шум)
  ...
  CabinClicks: Cardinal;
  // Канал для щелчков в кабине (395;254;контроллер;реверсор)
  Vstrech: Cardinal; // Канал для звука встречного поезда
  StukTrog: Cardinal; // Канал для удара сцепки при трограньи на Электричке
  IMRZachelka: Cardinal;
  // Канал для звука щелчка ЭМ-защелки №304 на Чехах (кроме ЧС2к)
  ...
  // Канал для пиканья КЛУБ-а при приближении к ограничениею
  LocoPowerEquipment: Cardinal;
  // Канал для звука силового оборудования локомотива(БВ, ФР, Жалюзи)
  FrontTP_Channel, BackTP_Channel: Cardinal;
  // Каналы для звуков поднятия (опускания) токоприёмника
  ...
  // Флаг для воспроизведения режима автоведения САУТ
  ...
  // Флаг для воспроизведения звуков вентиляторов (внешних)
  ...
  isPlayBrake: Boolean;
  // Флаг для воспроизведения звука трения колодок при торможении
  isPlayStochist: Boolean; // Флаг для воспроизведения звука дворников
  isPlayStochistUdar: Boolean;
  // Флаг для воспроизведения звука удара о край стекла удара дворников
  ...
  // Флаг для воспроизведения звуков силового оборудования локомотива(БВ, ФР)
  isPlayCabinClicks: Boolean;
  // Флаг для воспроизведения кабинных щелчков(395;254;контроллер;реверсор)
  isPlayIMRZachelka: Boolean;
  // Флаг для воспроизведения звков щелчка ЭМ-защелки реверсивки
...
const
  ...
  // ------------------------------------------------------------------------------//
procedure PlayPerestukIsEnd(vHandle, vStream, vData: Cardinal;
  vUser: Pointer); stdcall;
begin
...
// ------------------------------------------------------------------------------//
procedure PlaySAVPEINFOIsEnd(vHandle, vStream, vData: Cardinal;
  vUser: Pointer); stdcall;
begin
...
// ------------------------------------------------------------------------------//
procedure PlayCompressorIsEnd(vHandle, vStream, vData: Cardinal;
  vUser: Pointer); stdcall;
begin
...
// ------------------------------------------------------------------------------//
procedure PlayXCompressorIsEnd(vHandle, vStream, vData: Cardinal;
  vUser: Pointer); stdcall;
begin
...
function GetChannelRemaindPlayTime2Sec(var chan: Cardinal): double;
begin
  Result := BASS_ChannelBytes2Seconds(chan, BASS_ChannelGetLength(chan,
    BASS_POS_BYTE) - BASS_ChannelGetPosition(chan, BASS_POS_BYTE));
end;
...
begin
  ...
  begin
    ...
    BASS_ChannelSetAttribute(Vent_Channel_FX, BASS_ATTRIB_TEMPO_PITCH,
      VentPitch);
    BASS_ChannelSetAttribute(XVent_Channel_FX, BASS_ATTRIB_TEMPO_PITCH,
      VentPitch);
    BASS_ChannelSetAttribute(VentCycle_Channel_FX, BASS_ATTRIB_TEMPO_PITCH,
      VentPitch);
    BASS_ChannelSetAttribute(XVentCycle_Channel_FX, BASS_ATTRIB_TEMPO_PITCH,
      VentPitch);
  end;
  ...
  begin
    ...
    BASS_ChannelSetAttribute(VentTD_Channel_FX, BASS_ATTRIB_TEMPO_PITCH,
      VentTDPitch);
    BASS_ChannelSetAttribute(XVentTD_Channel_FX, BASS_ATTRIB_TEMPO_PITCH,
      VentTDPitch);
    BASS_ChannelSetAttribute(VentCycleTD_Channel_FX, BASS_ATTRIB_TEMPO_PITCH,
      VentTDPitch);
    BASS_ChannelSetAttribute(XVentCycleTD_Channel_FX, BASS_ATTRIB_TEMPO_PITCH,
      VentTDPitch);
  end;
end;
...
begin
  try
    ...
    LocoChannel[ChannelNum] := BASS_StreamCreateFile(False, FileName, 0, 0,
      LOOP_FLAG);
...
begin
  ...
  begin
    try
      ...
      if Loop = True then
        Unipuls_Channel[UnipulsChanNum] := BASS_StreamCreateFile(False,
          FileName, 0, 0, LOOP_FLAG);
      if Loop = False then
        Unipuls_Channel[UnipulsChanNum] := BASS_StreamCreateFile(False,
          FileName, 0, 0, DEFAULT_FLAG);
      BASS_ChannelSetAttribute(Unipuls_Channel[UnipulsChanNum],
        BASS_ATTRIB_VOL, 0);
      ...
      begin
        BASS_ChannelSlideAttribute(Unipuls_Channel[0], BASS_ATTRIB_VOL,
          UnipulsVol1 / 100, 500);
        BASS_ChannelSlideAttribute(Unipuls_Channel[1], BASS_ATTRIB_VOL,
          0, 1000);
      end
      else
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        BASS_ChannelSetAttribute(LocoChannel[0], BASS_ATTRIB_VOL,
          trcBarLocoPerestukVol.Position / 100);
        // Шум езды (в ст. вар. перестук) [1]
        BASS_ChannelSetAttribute(LocoChannel[1], BASS_ATTRIB_VOL,
          trcBarLocoPerestukVol.Position / 100);
        // Шум езды (в ст. вар. перестук) [2]
        BASS_ChannelSetAttribute(LocoChannelPerestuk, BASS_ATTRIB_VOL,
          trcBarLocoPerestukVol.Position / 100);
        if cbExtIntSounds.Checked = False then
          BASS_ChannelSetAttribute(WagChannel, BASS_ATTRIB_VOL, 0)
        else
          BASS_ChannelSetAttribute(WagChannel, BASS_ATTRIB_VOL,
            trcBarWagsVol.Position / 60);
        BASS_ChannelSetAttribute(PRSChannel, BASS_ATTRIB_VOL,
          trcBarPRSVol.Position / 100);
        BASS_ChannelSetAttribute(CabinClicks, BASS_ATTRIB_VOL,
          trcBarLocoClicksVol.Position / 100);
        BASS_ChannelSetAttribute(RB_Channel, BASS_ATTRIB_VOL,
          trcBarLocoClicksVol.Position / 100);
        BASS_ChannelSetAttribute(Vstrech, BASS_ATTRIB_VOL, 0.5);
        BASS_ChannelSetAttribute(PickKLUBChannel, BASS_ATTRIB_VOL,
          trcBarLocoClicksVol.Position / 100);
        BASS_ChannelSetAttribute(SAUTChannelObjects, BASS_ATTRIB_VOL,
          trcBarSAVPVol.Position / 100);
        BASS_ChannelSetAttribute(SAUTChannelObjects2, BASS_ATTRIB_VOL,
          trcBarSAVPVol.Position / 100);
        BASS_ChannelSetAttribute(SAUTChannelZvonok, BASS_ATTRIB_VOL,
          trcBarNatureVol.Position / 100);
        BASS_ChannelSetAttribute(Unipuls_Channel[0], BASS_ATTRIB_VOL,
          trcBarVspomMahVol.Position / 100);
        BASS_ChannelSetAttribute(Unipuls_Channel[1], BASS_ATTRIB_VOL,
          trcBarVspomMahVol.Position / 100);
        BASS_ChannelSetAttribute(IMRZachelka, BASS_ATTRIB_VOL,
          trcBarLocoClicksVol.Position / 100);
        BASS_ChannelSetAttribute(ClockChannel, BASS_ATTRIB_VOL,
          trcBarLocoClicksVol.Position / 100);
        BASS_ChannelSetAttribute(Stochist_Channel, BASS_ATTRIB_VOL,
          trcBarVspomMahVol.Position / 100);
        BASS_ChannelSetAttribute(StochistUdar_Channel, BASS_ATTRIB_VOL,
          trcBarVspomMahVol.Position / 100);
        BASS_ChannelSetAttribute(VentTD_Channel_FX, BASS_ATTRIB_VOL, VentTDVol);
        BASS_ChannelSetAttribute(VentCycleTD_Channel_FX, BASS_ATTRIB_VOL,
          VentTDVol);
        BASS_ChannelSetAttribute(Rain_Channel, BASS_ATTRIB_VOL,
          trcBarNatureVol.Position / 100);
        if ChannelNumTED = 1 then
          BASS_ChannelSetAttribute(TEDChannel_FX, BASS_ATTRIB_VOL,
            TEDVlm * 0.85);
        ...
        begin
          Case ChannelNumDiz Of
            1:
              BASS_ChannelSetAttribute(DizChannel, BASS_ATTRIB_VOL,
                trcBarDieselVol.Position / 100);
            0:
              BASS_ChannelSetAttribute(DizChannel2, BASS_ATTRIB_VOL,
                trcBarDieselVol.Position / 100);
          end;
        end
        else
        ...
        begin
          ...
          BASS_ChannelSetAttribute(Compressor_Channel, BASS_ATTRIB_VOL,
            trcBarVspomMahVol.Position / 125);
          BASS_ChannelSetAttribute(CompressorCycleChannel, BASS_ATTRIB_VOL,
            trcBarVspomMahVol.Position / 125);
      ...
      begin
        ...
        BASS_ChannelSetAttribute(LocoChannelPerestuk, BASS_ATTRIB_VOL,
          trcBarLocoPerestukVol.Position / 100);
        BASS_ChannelSetAttribute(WagChannel, BASS_ATTRIB_VOL,
          trcBarWagsVol.Position / 100);
        BASS_ChannelSetAttribute(PRSChannel, BASS_ATTRIB_VOL,
          trcBarPRSVol.Position / 200);
        BASS_ChannelSetAttribute(Rain_Channel, BASS_ATTRIB_VOL,
          trcBarNatureVol.Position / 100);
        ...
        begin
          ...
          BASS_ChannelSetAttribute(XVent_Channel_FX, BASS_ATTRIB_VOL,
            trcBarVspomMahVol.Position / 100);
          BASS_ChannelSetAttribute(XVentCycle_Channel_FX, BASS_ATTRIB_VOL,
            trcBarVspomMahVol.Position / 100);
          BASS_ChannelSetAttribute(XVentTD_Channel_FX, BASS_ATTRIB_VOL,
            VentTDVol);
          BASS_ChannelSetAttribute(XVentCycleTD_Channel_FX, BASS_ATTRIB_VOL,
            VentTDVol);
        end
        else
        begin
          BASS_ChannelSetAttribute(VentTD_Channel_FX, BASS_ATTRIB_VOL,
            VentTDVol);
          BASS_ChannelSetAttribute(VentCycleTD_Channel_FX, BASS_ATTRIB_VOL,
            VentTDVol);
          if Loco <> 'ED4M' then
        ...
        begin
          ...
          BASS_ChannelSetAttribute(XCompressor_Channel, BASS_ATTRIB_VOL,
            trcBarVspomMahVol.Position / 100);
          BASS_ChannelSetAttribute(XCompressorCycleChannel, BASS_ATTRIB_VOL,
            trcBarVspomMahVol.Position / 100);
        end
        else
        begin
        ...
        BASS_ChannelSetAttribute(ClockChannel, BASS_ATTRIB_VOL,
          trcBarLocoClicksVol.Position / 200);
        BASS_ChannelSetAttribute(Stochist_Channel, BASS_ATTRIB_VOL,
          trcBarVspomMahVol.Position / 200);
        BASS_ChannelSetAttribute(StochistUdar_Channel, BASS_ATTRIB_VOL,
          trcBarVspomMahVol.Position / 200);
      end;
    end;
  ...
  begin
    ...
    begin
      try
        ...
        LocoChannelPerestuk := BASS_StreamCreateFile(False, LocoPerestukF, 0, 0,
          DEFAULT_FLAG);
        isPlayPerestuk := True;
        ...
        if (Camera <> 2) or (Loco = 'ED4M') then
          ...
          BASS_ChannelSetAttribute(LocoChannelPerestuk, BASS_ATTRIB_VOL, 0);
        BASS_ChannelSetSync(LocoChannelPerestuk, BASS_SYNC_END, 0,
          @PlayPerestukIsEnd, nil);
      except
      end;
    end;
    // === ПЕРЕСТУК ВАГОНОВ === //
    ...
    // === САУТ Объекты [1] === //
    if (isPlaySAUTObjects = False) and
      (BASS_ChannelIsActive(SAUTChannelObjects) = 0) then
    begin
      try
        ...
        begin
          SAUTChannelObjects := BASS_StreamCreateFile(False, SAUTF, 0, 0,
            DEFAULT_FLAG);
        end
        else
        begin
          SAUTChannelObjects := BASS_StreamCreateFile(True, ResPotok.Memory, 0,
            ResPotok.Size, DEFAULT_FLAG);
        end;
    ...
    begin
      try
        ...
        if PlayResFlag = False then
        begin
          SAUTChannelObjects2 := BASS_StreamCreateFile(False, SAUTF, 0, 0,
            DEFAULT_FLAG);
        end
        else
        begin
          SAUTChannelObjects2 := BASS_StreamCreateFile(True, ResPotok.Memory, 0,
            ResPotok.Size, DEFAULT_FLAG);
        end;
    ...
    begin
      try
        ...
        SAUTChannelZvonok := BASS_StreamCreateFile(False, SAUTF, 0, 0,
          LOOP_FLAG);
    ...
    begin
      try
        ...
        if (isCameraInCabin = False) Or (Loco = 'ED4M') then
          BASS_ChannelSetAttribute(StukTrog, BASS_ATTRIB_VOL,
            0.01 * trcBarLocoPerestukVol.Position)
        else
    ...
    begin
      try
        ...
        CabinClicks := BASS_StreamCreateFile(False, CabinClicksF, 0, 0,
          DEFAULT_FLAG);
        BASS_ChannelPlay(CabinClicks, True);
        isPlayCabinClicks := True;
        if Camera = 0 then
          BASS_ChannelSetAttribute(CabinClicks, BASS_ATTRIB_VOL,
            0.01 * trcBarLocoClicksVol.Position)
        else
          BASS_ChannelSetAttribute(CabinClicks, BASS_ATTRIB_VOL, 0);
      except
      end;
    ...
    begin
      try
        ...
        Vigilance_Check_Channel := BASS_StreamCreateFile(False,
          PChar('TWS/KLUB_beep.wav'), 0, 0, DEFAULT_FLAG);
        BASS_ChannelPlay(Vigilance_Check_Channel, True);
        isPlayVcheck := True;
        BASS_ChannelSetAttribute(Vigilance_Check_Channel, BASS_ATTRIB_VOL,
          0.01 * trcBarLocoClicksVol.Position);
      except
      end;
    end;
    ...
    begin
      try
        ...
        TEDChannel := BASS_StreamCreateFile(False, TEDF, 0, 0,
          DECODE_FLAG or BASS_UNICODE);
    ...
    begin
      try
        ...
        ReduktorChannel := BASS_StreamCreateFile(False, ReduktorF, 0, 0,
          DECODE_FLAG);
        ReduktorChannel_FX := BASS_FX_TempoCreate(ReduktorChannel,
          BASS_FX_FREESOURCE);
        BASS_ChannelFlags(ReduktorChannel_FX, BASS_SAMPLE_LOOP,
          BASS_SAMPLE_LOOP);
    ...
    begin
      try
        ...
        IMRZachelka := BASS_StreamCreateFile(False, IMRZashelka, 0, 0,
          DEFAULT_FLAG);
    ...
    begin
      try
        ...
        FrontTP_Channel := BASS_StreamCreateFile(False, FTPF, 0, 0,
          DEFAULT_FLAG);
        isPlayFTP := True;
        BASS_ChannelPlay(FrontTP_Channel, True);
        BASS_ChannelSetAttribute(FrontTP_Channel, BASS_ATTRIB_VOL,
          0.01 * trcBarVspomMahVol.Position);
      except
      end;
    end;
    // === ЗАДНИЙ ТП === //
    ...
    begin
      try
        ...
        BackTP_Channel := BASS_StreamCreateFile(False, BTPF, 0, 0,
          DEFAULT_FLAG);
        isPlayBTP := True;
        BASS_ChannelPlay(BackTP_Channel, True);
        BASS_ChannelSetAttribute(BackTP_Channel, BASS_ATTRIB_VOL,
          0.005 * trcBarVspomMahVol.Position);
      except
      end;
    end;
    // === МК === //
    ...
    begin
      try
        ...
        Compressor_Channel := BASS_StreamCreateFile(False, CompressorF, 0, 0,
          DEFAULT_FLAG);
        ...
        if AnsiCompareStr(CompressorCycleF, '') <> 0 then
          BASS_ChannelSetSync(Compressor_Channel, BASS_SYNC_END, 0,
            @PlayCompressorIsEnd, nil);
      except
      end;
    end;
    // === МК [ВНЕШНИЙ] === //
    ...
    begin
      try
        ...
        XCompressor_Channel := BASS_StreamCreateFile(False, XCompressorF, 0, 0,
          DEFAULT_FLAG);
        ...
        if AnsiCompareStr(XCompressorCycleF, '') <> 0 then
          BASS_ChannelSetSync(XCompressor_Channel, BASS_SYNC_END, 0,
            @PlayXCompressorIsEnd, nil)
      except
      end;
    end;
    // === МК [ЦИКЛ] === //
    ...
    begin
      try
        BASS_ChannelStop(CompressorCycleChannel);
        BASS_StreamFree(CompressorCycleChannel);
        CompressorCycleChannel := BASS_StreamCreateFile(False, CompressorCycleF,
          0, 0, LOOP_FLAG);
    ...
    // === МК [ЦИКЛ] [ВНЕШНИЙ] === //
    if (isPlayXCompressorCycle = False) and
      (AnsiCompareText(XCompressorCycleF, '') <> 0) then
    begin
      try
        BASS_ChannelStop(XCompressorCycleChannel);
        BASS_StreamFree(XCompressorCycleChannel);
        XCompressorCycleChannel := BASS_StreamCreateFile(False,
          XCompressorCycleF, 0, 0, LOOP_FLAG);
    ...
    begin
      try
        ...
        if StopVent = False then
          Vent_Channel := BASS_StreamCreateFile(False, VentStartF, 0, 0,
            DECODE_FLAG)
        else
          Vent_Channel := BASS_StreamCreateFile(False, VentStopF, 0, 0,
            DECODE_FLAG);
        Vent_Channel_FX := BASS_FX_TempoCreate(Vent_Channel,
          BASS_FX_FREESOURCE);
    ...
    begin
      try
        ...
        if StopVent = False then
          XVent_Channel := BASS_StreamCreateFile(False, XVentStartF, 0, 0,
            DECODE_FLAG)
        else
          XVent_Channel := BASS_StreamCreateFile(False, XVentStopF, 0, 0,
            DECODE_FLAG);
        XVent_Channel_FX := BASS_FX_TempoCreate(XVent_Channel,
          BASS_FX_FREESOURCE);
    ...
    begin
      try
        ...
        VentCycle_Channel := BASS_StreamCreateFile(False, VentCycleF, 0, 0,
          DECODE_FLAG);
        VentCycle_Channel_FX := BASS_FX_TempoCreate(VentCycle_Channel,
          BASS_FX_FREESOURCE);
        BASS_ChannelFlags(VentCycle_Channel_FX, BASS_SAMPLE_LOOP,
          BASS_SAMPLE_LOOP);
    ...
    begin
      try
        ...
        XVentCycle_Channel := BASS_StreamCreateFile(False, XVentCycleF, 0, 0,
          DECODE_FLAG);
        XVentCycle_Channel_FX := BASS_FX_TempoCreate(XVentCycle_Channel,
          BASS_FX_FREESOURCE);
        BASS_ChannelFlags(XVentCycle_Channel_FX, BASS_SAMPLE_LOOP,
          BASS_SAMPLE_LOOP);
    ...
    begin
      try
        ...
        VentTD_Channel := BASS_StreamCreateFile(False, VentTDF, 0, 0,
          DECODE_FLAG);
        VentTD_Channel_FX := BASS_FX_TempoCreate(VentTD_Channel,
          BASS_FX_FREESOURCE);
    ...
    begin
      try
        ...
        XVentTD_Channel := BASS_StreamCreateFile(False, XVentTDF, 0, 0,
          DECODE_FLAG);
        XVentTD_Channel_FX := BASS_FX_TempoCreate(XVentTD_Channel,
          BASS_FX_FREESOURCE);
    ...
    begin
      try
        ...
        VentCycleTD_Channel := BASS_StreamCreateFile(False, VentCycleTDF, 0, 0,
          DECODE_FLAG);
        VentCycleTD_Channel_FX := BASS_FX_TempoCreate(VentCycleTD_Channel,
          BASS_FX_FREESOURCE);
        BASS_ChannelFlags(VentCycleTD_Channel_FX, BASS_SAMPLE_LOOP,
          BASS_SAMPLE_LOOP);
    ...
    begin
      try
        ...
        XVentCycleTD_Channel := BASS_StreamCreateFile(False, XVentCycleTDF, 0,
          0, DECODE_FLAG);
        XVentCycleTD_Channel_FX := BASS_FX_TempoCreate(XVentCycleTD_Channel,
          BASS_FX_FREESOURCE);
        BASS_ChannelFlags(XVentCycleTD_Channel_FX, BASS_SAMPLE_LOOP,
          BASS_SAMPLE_LOOP);
    ...
    begin
      try
        ...
        Brake254_Channel[0] := BASS_StreamCreateFile(False, Brake254F, 0, 0,
          DECODE_FLAG);
        Brake254_Channel_FX[0] := BASS_FX_TempoCreate(Brake254_Channel[0],
          BASS_FX_FREESOURCE);
    ...
    begin
      try
        ...
        BASS_ChannelStop(Brake254_Channel_FX[1]);
        BASS_StreamFree(Brake254_Channel_FX[1]);
        Brake254_Channel[1] := BASS_StreamCreateFile(False, CycleBrake254F, 0,
          0, DECODE_FLAG);
        Brake254_Channel_FX[1] := BASS_FX_TempoCreate(Brake254_Channel[1],
          BASS_FX_FREESOURCE);
        BASS_ChannelFlags(Brake254_Channel_FX[1], BASS_SAMPLE_LOOP,
          BASS_SAMPLE_LOOP);
    ...
    // === ТРЕНИЕ КОЛОДОК === //
    if isPlayBrake = False then
    begin
      try
        BASS_ChannelStop(Brake_Channel[0]);
        BASS_StreamFree(Brake_Channel[0]);
        BASS_ChannelStop(Brake_Channel[1]);
        BASS_StreamFree(Brake_Channel[1]);
        Brake_Channel[0] := BASS_StreamCreateFile(False, BrakeF, 0, 0,
          LOOP_FLAG);
        Brake_Channel[1] := BASS_StreamCreateFile(False,
          PChar('TWS/x_brake_slipp.wav'), 0, 0, LOOP_FLAG);
        isPlayBrake := True;

        var
        brakeVolume := BrakeCylinders * Speed *
          trcBarLocoPerestukVol.Position / 144000;
        if isCameraInCabin = False then
          brakeVolume := 0.5 * brakeVolume;
        if EDTAmperage <> 0 then
          brakeVolume := 0.25 * brakeVolume;

        BASS_ChannelSetAttribute(Brake_Channel[1], BASS_ATTRIB_VOL,
          brakeVolume);
    ...
    begin
      try
        ...
        isPlayRain := True;
        BASS_ChannelSetAttribute(Rain_Channel, BASS_ATTRIB_VOL,
          0.01 * trcBarNatureVol.Position);
      except
      end;
    end;
    // === ХОДЬБА ПО СНЕГУ === //
    if (isPlayWalkSound = True) and (BASS_ChannelIsActive(WalkSoundChannel) = 0)
    then
    begin
      // BASS_ChannelStop(WalkSoundChannel); BASS_StreamFree(WalkSoundChannel);
      WalkSoundChannel := BASS_StreamCreateFile(False, WalkSoundF, 0, 0,
        LOOP_FLAG);
      BASS_ChannelSetAttribute(WalkSoundChannel, BASS_ATTRIB_VOL,
        0.01 * trcBarNatureVol.Position);
    ...
    begin
      try
        ...
        BeltPool_Channel := BASS_StreamCreateFile(False,
          PChar('TWS/belt_pul.wav'), 0, 0, LOOP_FLAG);
        BASS_ChannelPlay(BeltPool_Channel, True);
        isPlayBeltPool := True;
        BASS_ChannelSetAttribute(BeltPool_Channel, BASS_ATTRIB_VOL,
          0.01 * trcBarLocoClicksVol.Position);
      except
      end;
    end;
    ...
    begin
      ...
      NatureChannel_FX := BASS_FX_TempoCreate(NatureChannel,
        BASS_FX_FREESOURCE);
    ...
    begin
      try
        ...
        Stochist_Channel := BASS_StreamCreateFile(False, StochistF, 0, 0,
          LOOP_FLAG);
    ...
    begin
      try
        ...
        StochistUdar_Channel := BASS_StreamCreateFile(False,
          PChar('TWS/stochist_udar.wav'), 0, 0, DEFAULT_FLAG);
    ...
    // === КЛАВИАТУРА КЛУБ-у === //
    if ((cbKLUBSounds.Checked = True) and (KLUBOpen = 1) and (prevKeyLKM = 0)
      and (getasynckeystate(1) <> 0)) then
    begin
      try
        BASS_ChannelStop(PickKLUBChannel);
        BASS_StreamFree(PickKLUBChannel);
        PickKLUBChannel := BASS_StreamCreateFile(False,
          PChar('TWS/KLUB_pick.wav'), 0, 0, DEFAULT_FLAG);
    ...
    begin
      try
        ...
        KLUB_BEEP := BASS_StreamCreateFile(False, PChar('TWS/KLUB_beep.wav'), 0,
          0, DEFAULT_FLAG);
        BASS_ChannelPlay(KLUB_BEEP, True);
        BASS_ChannelSetAttribute(KLUB_BEEP, BASS_ATTRIB_VOL,
          0.01 * trcBarLocoClicksVol.Position);
      except
      end;
    end;
    ...
    begin
      try
        ...
        isPlayOgrSpKlub := -1;
        Ogr_Speed_KLUB := BASS_StreamCreateFile(False,
          PChar('TWS/KLUB_pick.wav'), 0, 0, LOOP_FLAG);
        BASS_ChannelPlay(Ogr_Speed_KLUB, True);
        BASS_ChannelSetAttribute(Ogr_Speed_KLUB, BASS_ATTRIB_VOL,
          0.01 * trcBarLocoClicksVol.Position);
      except
      end;
    end;
    ...
    begin
      try
        ...
        LocoPowerEquipment := BASS_StreamCreateFile(False, LocoPowerEquipmentF,
          0, 0, DEFAULT_FLAG);
        BASS_ChannelPlay(LocoPowerEquipment, True);
        BASS_ChannelSetAttribute(LocoPowerEquipment, BASS_ATTRIB_VOL,
          0.01 * trcBarVspomMahVol.Position);
      except
      end;
    end;
