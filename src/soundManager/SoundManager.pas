...
begin
    ...
    if isPlayPerestuk=False then begin
       try
          ...
          LocoChannelPerestuk := BASS_StreamCreateFile(FALSE, LocoPerestukF, 0, 0, DEFAULT_FLAG); isPlayPerestuk:=True;
    ...
    if (isPlaySAUTObjects=False) and (BASS_ChannelIsActive(SAUTChannelObjects)<>0) then begin
       try
          ...
          isPlaySAUTObjects:=True; PlayRESFlag:=False;
          if isCameraInCabin=True then
...
//    end;
    ...
    if isPlayVstrech = False then begin
       try
          ...
          var vstrechVolume: Double := 0.01 * trcBarNatureVol.Position;
          if isCameraInCabin = False then vstrechVolume := trcBarNatureVol.Position / 175;
    ...
    if isPlayIMRZachelka=False then begin
       try
          ...
          var emVolume: Double := 0.01 * trcBarLocoClicksVol.Position;
          if Camera <> 0 then emVolume := 0;
...
//    end;
    ...
    if isPlayRB=False then begin
       try
          ...
          var rbVolume: Double := 0.01 * trcBarLocoClicksVol.Position;
          if Camera <> 0 then rbVolume := 0;
    ...
    if isPlayCompressor=False then begin
       try
          ...
          isPlayCompressor:=True; Inc(CameraX);
          BASS_ChannelSetAttribute(Compressor_Channel, BASS_ATTRIB_VOL, 0);
    ...
    if (isPlayXCompressor=False) then begin
       try
          ...
          isPlayXCompressor:=True; Inc(CameraX);
          BASS_ChannelSetAttribute(XCompressor_Channel, BASS_ATTRIB_VOL, 0);
    ...
    if isPlayCompressorCycle=False then begin
       try
          ...
          isPlayCompressorCycle:=True; Inc(CameraX);
          BASS_ChannelSetAttribute(CompressorCycleChannel, BASS_ATTRIB_VOL, 0);
          CompressorCycleF:=PChar('');
       except end;
    end;
    ...
    if (isPlayXCompressorCycle=False) and (AnsiCompareText(XCompressorCycleF, '')<>0) then begin
       try
          ...
          isPlayXCompressorCycle:=True; Inc(CameraX);
          BASS_ChannelSetAttribute(XCompressorCycleChannel, BASS_ATTRIB_VOL, 0);
          XCompressorCycleF:=PChar('');
       except end;
    end;
...
//          end;
    ...
    if isPlayBrake=False then begin
       try
          ...
          Brake_Channel[0] := BASS_StreamCreateFile(FALSE, BrakeF, 0, 0, LOOP_FLAG);
          Brake_Channel[1] := BASS_StreamCreateFile(FALSE, PChar('TWS/x_brake_slipp.wav'), 0, 0, LOOP_FLAG);
          isPlayBrake := True;

          var brake_volume := BrakeCylinders * Speed * trcBarLocoPerestukVol.Position / 144000;
          if isCameraInCabin = False then brake_volume := 0.5 * brake_volume;
          if EDTAmperage <> 0 then brake_volume := 0.25 * brake_volume;

          BASS_ChannelSetAttribute(Brake_Channel[1], BASS_ATTRIB_VOL, brake_volume);
    ...
    if isPlayClock=False then begin
       try
          ...
          if Speed <= 2 then
             ClockChannel := BASS_StreamCreateFile(FALSE, PChar('TWS/clock.wav'), 0, 0, LOOP_FLAG);
          if Speed > 2 then
             ClockChannel := BASS_StreamCreateFile(FALSE, PChar('TWS/skorostemer.wav'), 0, 0, LOOP_FLAG);
          BASS_ChannelPlay(ClockChannel, True);
          isPlayClock:=True;
          var clockVolume: Double := 0;
          if Camera = 0 then clockVolume := 0.01 * trcBarLocoClicksVol.Position;
    ...
    if isPlayStochist=False then begin
       try
          ...
          isPlayStochist:=True;
          var stochistVolume: Double := 0;
          if Camera=0 then stochistVolume := 0.01 * trcBarVspomMahVol.Position;
    ...
    if ((cbKLUBSounds.Checked=True) and (KLUBOpen=1) and (prevKeyLKM=0) and (getasynckeystate(1) <> 0)) then begin
       try
          ...
          var klubVolume: Double := 0;
          if Camera=0 then klubVolume := 0.01 * trcBarLocoClicksVol.Position;
