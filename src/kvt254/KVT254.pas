...
var
  ...
  singleTemp2: Single;
begin
  if StopBrake254 = False then
  begin
    if BASS_ChannelIsActive(Brake254_Channel_FX[0]) <> 0 then
    begin
      Brake254TimeLeft := BASS_ChannelBytes2Seconds(Brake254_Channel_FX[0],
        BASS_ChannelGetLength(Brake254_Channel_FX[0], BASS_POS_BYTE) -
        BASS_ChannelGetPosition(Brake254_Channel_FX[0], BASS_POS_BYTE));
      if (Brake254TimeLeft <= 0.2) and
        (BASS_ChannelIsActive(Brake254_Channel_FX[1]) = 0) then
        isPlayCycleBrake254 := False;
    end;
  end;

  Inc(TCIncrementer2);
  if (TCIncrementer2 > Trunc(300 / MainCycleFreq)) then
  begin
  ...
  begin
    ...
    begin
      if (BASS_ChannelIsActive(Brake254_Channel_FX[0]) = 0) Or
        ((BASS_ChannelIsActive(Brake254_Channel_FX[0]) <> 0) And
        (StopBrake254 = True)) then
      begin
  ...
  begin
    BASS_ChannelSetAttribute(Brake254_Channel_FX[0], BASS_ATTRIB_VOL, 0.3);
    Inc(TCIncrementer);
    if (TCIncrementer > Trunc(300 / MainCycleFreq)) and (StopBrake254 = False)
    then
    begin
      if (singleTemp1) < 0 then
      begin
